...
const destinations: { id: PageId; href: string; label: string; icon: typeof ChartColumn }[] = [
  ...
  { id: "compare", href: "/compare", label: "Compare", icon: Users },
...
// five destinations don't fit beside the plan header's brand at 375 px — so
...
export function PageNav({ current }: { current: PageId }) {
  ...
  if (current === "plan") {
    return (
      <nav className="flex items-center gap-1.5 sm:gap-2" aria-label="Pages">
        {pills}
      </nav>
    );
  }
  ...
  return (
    <nav className="flex items-center justify-between gap-2" aria-label="Pages">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">{pills}</div>
    </nav>
  );
}
